...
import type { Violacion } from '@anima/world'
import { Partida } from '@anima/perceive'

import { resumir, SECUENCIAS, TECHO_DE_LA_FRICCION } from '../src/index.js'
import type { FilaDelBanco, NombreDeSecuencia, Situaciones } from '../src/index.js'
...
import {
  ...
  TANQUE_LLENO,
  TICKS,
} from './el-banco-de-la-mente.js'
import type { Banco } from './el-banco-de-la-mente.js'
import {
  cuerpo,
  escenaDe,
  firmaDeSueltas,
...
// ─── El banco de la mente, repartido entre archivos ──────────────────────────
//
// La corrida canónica, su control del tanque lleno, la `Corrida` de una partida
// y las constantes del criterio VIVÍAN ACÁ y se mudaron a
// `el-banco-de-la-mente.ts`, con el mismo movimiento que ya se le hizo al
// control del azar (`azar.ts`): la unidad de paralelismo de vitest es el
// archivo, y las dos cohortes —40 partidas de 20.000 ticks— corrían secuenciales
// adentro de éste mientras cinco tandas del azar corrían en paralelo. Ahora las
// corren cinco tandas propias (`el-banco-tanda-N.test.ts`) y este archivo lee
// del disco lo que ya está y corre lo que falte. Ningún número cambia: la
// partida es función pura de `(semilla, tanque, tope)`, y la verificación fue
// renglón por renglón sobre las tablas del banco.
