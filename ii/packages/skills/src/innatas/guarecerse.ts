...
export const CONTRATO_GUARECERSE: Contrato = {
  ...
  huecos: [
    ...
    'CONSTRUIRSE UN TECHO SIGUE SIN EXISTIR, pero ya no por donde decía. El hueco anterior era `place(bp: Blueprint)` con `Blueprint = { id, at }`, sin forma de obtener uno y con `stepWorld` rechazándolo con `no-implementado`. Las dos mitades se cerraron en el Gate 5→6: `place(b, at, revision?)` DESPLIEGA un cuerpo ya armado y el mundo lo acepta (ADR II-0022), y el plano de verdad —`BlueprintDefinition`, canónico y versionado— vive en `@anima/physics`. Lo que falta ahora es otra cosa y hay que decirla con precisión: **armar la obra es encadenar `apply("union", …)` y esta habilidad no tiene con qué elegir el orden de las uniones**, que es lo que decide la forma (medido: `unir` siempre ata la parte 0, así que la topología sale del orden y no del plano). Y sigue sin haber una ley que haga que una obra puesta dé `sheltered`',
